#
# NOTE: This script is here for documentation. It does not run without the raw data (which is not included due to size constraints)
#
...
daily_data <- data_full %>% 
  ...
  mutate(remove = (minutes_on < 1439)  # - more than one minute missing
         | (minutes_lying > 100) # - more than 100 minutes with pod in horizontal position (70 to 100 degrees)
         | (max_temp - min_temp > 10))# - temperature difference of more than 10 degrees C

# Turn suspicious days to NA
daily_data <- bind_rows(filter(daily_data, !remove),
                        filter(daily_data, remove) %>% 
                          mutate(across(c("n_clicks", "dph", "n_encounters", "dps", "n_trains"), ~NA))
                        ) %>% 
  arrange(station, date)

write_csv(daily_data, "clean_data/daily_data.csv")
...
ggsave(cluster_fig, filename = "figs/fig_S4.tiff", width = 6, height = 4, dpi = 800)
